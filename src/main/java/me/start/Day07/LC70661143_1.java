...
public class LC70661143_1 {
    ...
    public int longestCommonSubsequence02(String text1, String text2) {
        //重点看优化为一维数组，此时使用两个变量作为存储左上角的元素，并在s1.charAt(i-1)==s2.charAt(j-1)的情况下把t/pre的值赋给dp[j];
        ...
        //因此要通过两个变量在每一次循环中不断更新左上角的值，并在s1.charAt(i-1)==s2.charAt(j-1)时把左上角的值(代码中为pre变量)赋给dp[j];
        int n = text1.length();
        int m = text2.length();
        int[] dp = new int[m + 1];
        for (int i = 1; i < n + 1; i++) {
